...
test.describe('User Overrides Persistence', () => {
    ...
    test('override changes recalculate summary and persist across reload', async ({ page }) => {
        ...
        // Register the save waiter BEFORE making the change so we cannot miss the response
        // if the debounce fires quickly. The debounce is 500ms, but under full-matrix load
        // the summary recalculation loop can run for several seconds, causing a race where
        // the response arrives before waitForResponse is registered if set up after the loop.
        const overridesSavedPromise = page.waitForResponse(
            (resp) => resp.url().includes('/api/overrides') && resp.status() === 200,
            { timeout: 15000 }
        );
        ...
        // Override recalculation is debounced (250ms) so wait for content to change.
        await expect(async () => {
        ...
        // Wait for the debounced server save (500ms) to complete
        await overridesSavedPromise;
