...
test.describe('Accessibility', () => {
    ...
    test.describe('WCAG 2.1 AA Compliance', () => {
        ...
        test('configuration section is accessible', async ({ page }) => {
            // Expand config panel (collapsed by default)
            await page.click('#configToggle');
            await expect(page.locator('#configContent')).toBeVisible();
        ...
        test('overrides page meets accessibility standards', async ({ page }) => {
            ...
            // Wait for CSS animations (fadeIn 0.2s) to settle before scanning contrast
            await page.evaluate(() =>
    ...
    test.describe('Keyboard Navigation', () => {
        test('all interactive elements are keyboard accessible', async ({ page }) => {
            // Expand config panel so its controls are focusable
            await page.click('#configToggle');
            await expect(page.locator('#configContent')).toBeVisible();
        ...
        test('buttons can be activated with Enter and Space', async ({ page }) => {
            ...
            // Wait for MIN_LOADING_MS timer to re-enable the button.
            // freezeTime() freezes Date.now(), so renderLoading(false) always
            // schedules a ~120ms timeout before removing disabled.
            await expect(generateBtn).toBeEnabled({ timeout: 5000 });

            // Register a click listener to confirm keyboard activation fired.
            await page.evaluate(() => {
                (window as any).__btnClickFired = false;
                document.getElementById('generateBtn')?.addEventListener(
                    'click',
                    () => { (window as any).__btnClickFired = true; },
                    { once: true }
                );
            });

            // Test Enter key activates the button
            await generateBtn.focus();
            await page.keyboard.press('Enter');

            // Confirm the click event fired (Enter activated the button)
            await page.waitForFunction(
                () => (window as any).__btnClickFired === true,
                { timeout: 5000 }
            );

            // Wait for the triggered report generation to complete
    ...
    test.describe('Screen Reader Compatibility', () => {
        ...
        test('aria-expanded correctly reflects toggle state', async ({ page }) => {
            ...
            // After clicking again, should be collapsed
            await toggle.click();
            await expect(toggle).toHaveAttribute('aria-expanded', 'false');
        });
    ...
    test.describe('Focus Management', () => {
        test('focus is visible on interactive elements', async ({ page }) => {
            // Wait for the button to be enabled (it starts disabled during init)
            const generateBtn = page.locator('#generateBtn');
            await expect(generateBtn).toBeEnabled({ timeout: 5000 });
            await generateBtn.focus();
    ...
    test.describe('Text and Content', () => {
        ...
        test('images have alt text', async ({ page }) => {
            ...
            // App currently uses no <img> tags — assert this explicitly
            // If images are added, the loop below validates accessibility
            expect(count).toBe(0);
            ...
            for (let i = 0; i < count; i++) {
                ...
                // Informative images must have non-empty alt; decorative images need role="presentation"
                expect(
                    (alt !== null && alt.trim().length > 0) || role === 'presentation'
                ).toBeTruthy();
            }
        });
