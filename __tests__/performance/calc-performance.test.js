...
const PERF_MULTIPLIER = process.env.CI ? 7 : (isStrictPerfRun ? 1 : 15);
const SCALING_VARIANCE_MULTIPLIER = isStrictPerfRun ? 10 : 12;
    ...
    describe('Scalability Regression Tests', () => {
        test('should maintain linear scaling', () => {
            ...
            if (smallPerEntry > 0.01) {
                expect(largePerEntry).toBeLessThan(smallPerEntry * SCALING_VARIANCE_MULTIPLIER);
            } else {
...
describe('Calculation Engine Stress Tests', () => {
    ...
    // Run stress tests everywhere; CI uses a relaxed multiplier
    const STRESS_MULTIPLIER =
        process.env.RUN_STRESS_TESTS === 'true' ? 1 : PERF_MULTIPLIER;

    describe('Extended Stress Tests', () => {
