...
const isStrictPerfRun = process.env.RUN_PERF_TESTS === 'true';
...
describe('Cache Performance', () => {
    describe('Map Operations', () => {
        test('should handle large profile cache efficiently', () => {
            ...
            // 10000 lookups should be very fast, but allow extra headroom on shared non-perf runners
            const maxLookupDuration = process.env.CI ? 15 : (isStrictPerfRun ? 10 : 20);
            expect(lookupDuration).toBeLessThan(maxLookupDuration);
        });
        ...
        test('should handle nested Map (holidays) efficiently', () => {
            ...
            // 10000 nested lookups should be fast, but allow extra headroom on shared non-perf runners
            const maxNestedLookupDuration = process.env.CI ? 20 : (isStrictPerfRun ? 20 : 35);
            expect(lookupDuration).toBeLessThan(maxNestedLookupDuration);
        });
    });
});
