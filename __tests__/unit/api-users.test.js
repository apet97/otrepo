...
describe('API Users', () => {
  ...
  describe('fetchUsers', () => {
    ...
    it('should throw on 403 error to distinguish API failure from empty workspace', async () => {
      fetch.mockResolvedValueOnce(mockResponse(null, { ok: false, status: 403 }));

      // Assert immediately (before any async checkpoint) to prevent unhandled-rejection detection
      await expect(Api.fetchUsers('workspace_123')).rejects.toThrow('Failed to fetch workspace users');
    });

    it('should throw on 401 error to distinguish API failure from empty workspace', async () => {
      fetch.mockResolvedValueOnce(mockResponse(null, { ok: false, status: 401 }));

      await expect(Api.fetchUsers('workspace_123')).rejects.toThrow('Failed to fetch workspace users');
    });

    it('should throw on network error to distinguish API failure from empty workspace', async () => {
      fetch.mockRejectedValueOnce(new Error('Network error'));

      await expect(Api.fetchUsers('workspace_123')).rejects.toThrow();
    });
