...
describe('API Module - Core Behaviors', () => {
  ...
  it('fetchUsers throws on transport/auth failure to distinguish API errors from empty workspaces', async () => {
    mockFetch.mockResolvedValue(mockResponse(null, { ok: false, status: 500 }));

    // Use .catch(e => e) to avoid unhandled-rejection detection before the assertion
    const error = await Api.fetchUsers('ws_test').catch(e => e);
    await jest.runAllTimersAsync();
