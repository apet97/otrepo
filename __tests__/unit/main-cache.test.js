...
describe('Main handleGenerateReport cache behavior', () => {
  ...
  it('uses cached entries when user selects "use" (old cache triggers prompt)', async () => {
    ...
    const oldTimestamp = Date.now() - 60_000; // 60s old — triggers cache prompt
    jest.spyOn(store, 'getCachedReport').mockResolvedValue({ entries, timestamp: oldTimestamp });
    ...
    expect(uiMock.showCachePrompt).toHaveBeenCalledWith(60);
  ...
  it('fetches fresh entries when user declines cache', async () => {
    ...
    const oldTimestamp = Date.now() - 60_000; // 60s old — triggers cache prompt
    jest.spyOn(store, 'getCachedReport').mockResolvedValue({ entries: [{ id: 'old_entry' }], timestamp: oldTimestamp });
