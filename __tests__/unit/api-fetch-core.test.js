...
describe('API Fetch Core', () => {
  ...
  describe('fetchWithAuth - Authentication', () => {
    ...
    it('blocks requests to non-Clockify domains and throws (SSRF prevention)', async () => {
      store.claims = { ...store.claims, backendUrl: 'https://evil.example.com' };

      // .catch(e => e) converts rejection to resolution to avoid unhandled-rejection detection
      const error = await Api.fetchUsers('workspace_123').catch(e => e);
      await jest.runAllTimersAsync();

      // fetch must NOT have been called (no token exfiltration to evil domain)
      expect(fetch).not.toHaveBeenCalled();
      // fetchUsers throws to surface the failure (not silently return [])
      expect(error).toBeInstanceOf(Error);
      expect(error.message).toContain('Failed to fetch workspace users');
    });

    it('blocks requests to http:// Clockify URLs (HTTPS enforcement)', async () => {
      store.claims = { ...store.claims, backendUrl: 'http://api.clockify.me/api' };
    ...
    it('blocks requests to malformed URLs in transport path', async () => {
      store.claims = { ...store.claims, backendUrl: '://not-a-url' };
  ...
  describe('fetchWithAuth - HTTP Error Responses', () => {
    ...
    it('should throw on 500 Internal Server Error to surface the failure', async () => {
      fetch.mockResolvedValueOnce(mockResponse(null, { ok: false, status: 500 }));

      // .catch(e => e) converts rejection to resolved value to avoid unhandled-rejection detection
      const error = await Api.fetchUsers('workspace_123').catch(e => e);
      await jest.runAllTimersAsync();

      // fetchUsers throws to distinguish API failures from empty workspaces
      expect(error).toBeInstanceOf(Error);
      expect(error.message).toContain('Failed to fetch workspace users');
    });

    it('should throw on network errors to surface the failure', async () => {
  ...
  describe('fetchWithAuth - Store Claims Handling', () => {
    it('should throw when store.claims is undefined (invalid URL)', async () => {
      store.claims = undefined;

      const error = await Api.fetchUsers('workspace_123').catch(e => e);
      await jest.runAllTimersAsync();

      // No valid backendUrl → invalid URL → fetchWithAuth blocks → fetchUsers throws
      expect(error).toBeInstanceOf(Error);
      expect(error.message).toContain('Failed to fetch workspace users');
      expect(fetch).not.toHaveBeenCalled();
    });

    it('should throw when store.claims is null (invalid URL)', async () => {
      store.claims = null;
    ...
    it('should throw when store.claims.backendUrl is empty string (invalid URL)', async () => {
      store.claims = {
        workspaceId: 'ws_test',
        backendUrl: ''
      };
      ...
      // Empty backendUrl → relative URL → fetchWithAuth blocks → fetchUsers throws
      expect(error).toBeInstanceOf(Error);
      expect(error.message).toContain('Failed to fetch workspace users');
      expect(fetch).not.toHaveBeenCalled();
    });
...
describe('API Fetch Core - Mutation Killing Tests', () => {
  ...
  describe('Token and Header Mutations', () => {
    ...
    it('should block request when token is null (expired)', async () => {
      ...
      // fetchUsers then throws to surface the failure.
      const error = await Api.fetchUsers('workspace_123').catch(e => e);
