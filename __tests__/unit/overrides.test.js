...
describe('Overrides UI', () => {
  ...
  describe('zero-valued override fields (P2-B)', () => {
    it('renders "0" in capacity input when capacity is 0', () => {
      store.users = [{ id: 'user1', name: 'Alice' }];
      store.overrides = { user1: { capacity: 0 } };
      store.profiles = new Map();

      renderOverridesPage();

      const container = document.getElementById('overridesUserList');
      const input = container?.querySelector('input.override-input[data-field="capacity"]');
      expect(input?.value).toBe('0');
    });

    it('adds has-custom class when capacity is 0', () => {
      store.users = [{ id: 'user1', name: 'Alice' }];
      store.overrides = { user1: { capacity: 0 } };
      store.profiles = new Map();
      ...
      const container = document.getElementById('overridesUserList');
      const card = container?.querySelector('.override-user-card');
      expect(card?.classList.contains('has-custom')).toBe(true);
    });

    it('does NOT apply inherited class when capacity is 0', () => {
      store.users = [{ id: 'user1', name: 'Alice' }];
      store.overrides = { user1: { capacity: 0 } };
      store.profiles = new Map();
      ...
      const container = document.getElementById('overridesUserList');
      const input = container?.querySelector('input.override-input[data-field="capacity"]');
      expect(input?.classList.contains('inherited')).toBe(false);
    });

    it('shows copy-from-global button when capacity and multiplier are both 0', () => {
      store.users = [{ id: 'user1', name: 'Alice' }];
      store.overrides = { user1: { mode: 'perDay', capacity: 0, multiplier: 0 } };
      store.profiles = new Map();
      ...
      const container = document.getElementById('overridesUserList');
      const copyBtn = container?.querySelector('.copy-from-global-btn');
