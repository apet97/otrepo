...
describe('Encryption Utilities', () => {
    ...
    describe('deriveEncryptionKey', () => {
        ...
        it('coalesces concurrent key derivation for the same workspace', async () => {
            const [key1, key2] = await Promise.all([
                deriveEncryptionKey('workspace-concurrent'),
                deriveEncryptionKey('workspace-concurrent'),
            ]);
    ...
    describe('storeEncrypted / retrieveEncrypted', () => {
        ...
        it('returns null for malformed version-1 payloads missing encryption fields', async () => {
            localStorage.setItem('missing-fields', JSON.stringify({
                ct: 'ciphertext-without-iv',
                v: 1,
            }));

            const retrieved = await retrieveEncrypted('missing-fields', key);
