...
describe('Export Module', () => {
  ...
  describe('downloadCsv', () => {
    ...
    it('should use day key for CSV dates', () => {
      const createElementSpy = jest.spyOn(document, 'createElement');
      const mockLink = {
        setAttribute: jest.fn(),
        click: jest.fn(),
          ...
          days: new Map([
            ['2024-12-31', {
              entries: [{
                ...
                analysis: { regular: 1, overtime: 0, isBillable: true }
              }],
              meta: { capacity: 8, isHoliday: false, isNonWorking: false, isTimeOff: false, holidayName: '' }
            }]
          ])
        }
      ];
...
describe('CSV Format Specification', () => {
  ...
  describe('Character Encoding', () => {
    ...
    it('should use UTF-8 charset in Blob MIME type', () => {
      ...
      let blobType = null;
      global.Blob = jest.fn((_content, options) => {
  ...
  // Placeholder rows tested in report-to-csv.integration.test.js:257

  describe('UTF-8 BOM', () => {
    /**
     * SPECIFICATION: UTF-8 BOM for Excel Compatibility
     *
     * The CSV should start with a UTF-8 Byte Order Mark (BOM):
     * - Character: \uFEFF (zero-width no-break space)
     * - Purpose: Signals to Excel that the file is UTF-8 encoded
     * - Without BOM: Excel may misinterpret non-ASCII characters
     */
    ...
    it('should prepend UTF-8 BOM to CSV content', () => {
      ...
      // After BOM, first header should be 'Date'
      expect(csvContent.substring(1, 5)).toBe('Date');
    });
  });
});
...
describe('CSV Formula Injection Prevention - Extended', () => {
  ...
  describe('Risky Prefix Detection', () => {
    ...
    riskyPrefixes.forEach(({ name, example }) => {
...
describe('Complex CSV Data Handling', () => {
  ...
  describe('Multi-line Descriptions', () => {
    it('should escape descriptions with \\n (Unix line breaks)', () => {
      ...
      // Newlines are stripped before CSV escaping
      expect(csvContent).toContain('Line 1Line 2Line 3');
      expect(csvContent).not.toContain('\nLine 2');
    });
...
// ========================================================================
// NEW: Summary CSV & Detailed CSV tests
// ========================================================================
...
describe('downloadSummaryCsv', () => {
    ...
    const entries = [
      {
        ...
        taskId: 't1',
        taskName: 'Task 1',
        timeInterval: { start: '2025-01-15T09:00:00Z', end: '2025-01-15T17:00:00Z', duration: 'PT8H' },
        hourlyRate: { amount: 50 },
        billable: true,
      },
      {
        id: 'e2',
        ...
        timeInterval: { start: '2025-01-15T09:00:00Z', end: '2025-01-15T19:00:00Z', duration: 'PT10H' },
        hourlyRate: { amount: 40 },
        billable: false,
      },
    ];

    mockAnalysis = calculateAnalysis(entries, mockStore, { start: '2025-01-15', end: '2025-01-15' });
  });

  it('should produce correct summary CSV headers', async () => {
    const { downloadSummaryCsv } = await import('../../js/export.js');
    ...
    await downloadSummaryCsv(mockAnalysis, 'user');

    const lines = csvContent.split('\n').filter(l => l.trim());
    const headerLine = lines[0].replace(/^\uFEFF/, ''); // strip BOM
    expect(headerLine).toContain('Group');
    expect(headerLine).toContain('Capacity');
    expect(headerLine).toContain('Regular');
    expect(headerLine).toContain('Overtime');
    expect(headerLine).toContain('DailyOvertime');
    expect(headerLine).toContain('OTPremium');
    // 19 header columns
    expect(headerLine.split(',').length).toBe(19);
  });

  it('should have one row per user when grouped by user', async () => {
    const { downloadSummaryCsv } = await import('../../js/export.js');
  ...
  it('should use default filename otplus-summary.csv', async () => {
    const { downloadSummaryCsv } = await import('../../js/export.js');
    ...
    const downloadCall = mockLink.setAttribute.mock.calls.find(c => c[0] === 'download');
    expect(downloadCall[1]).toBe('otplus-summary.csv');
  });
  ...
  it('should accept custom filename', async () => {
    ...
    await downloadSummaryCsv(mockAnalysis, 'user', 'my-export.csv');

    const downloadCall = mockLink.setAttribute.mock.calls.find(c => c[0] === 'download');
    expect(downloadCall[1]).toBe('my-export.csv');
  });

  it('should sanitize formula injection in group names', async () => {
    const { downloadSummaryCsv } = await import('../../js/export.js');

    // Create analysis with a malicious name
    const entries = [
      {
        id: 'e_evil',
        userId: 'evil_user',
        userName: '=cmd|calc|A1',
        description: 'Evil',
        timeInterval: { start: '2025-01-15T09:00:00Z', end: '2025-01-15T17:00:00Z', duration: 'PT8H' },
        hourlyRate: { amount: 50 },
        billable: true,
    ...
    const evilStore = createMockStore({ users: [{ id: 'evil_user', name: '=cmd|calc|A1' }] });
    const evilAnalysis = calculateAnalysis(entries, evilStore, { start: '2025-01-15', end: '2025-01-15' });
    ...
    await downloadSummaryCsv(evilAnalysis, 'user');

    // The leading = should be prefixed with '
    const dataLines = csvContent.split('\n').filter(l => l.trim());
    const dataRow = dataLines[1];
    expect(dataRow).toContain("'=cmd|calc|A1");
  });
});

describe('downloadDetailedCsv', () => {
  let mockStore;
  let mockAnalysis;
  ...
  beforeEach(async () => {
    ...
    const entries = [
      ...
      },
      {
        ...
        projectName: 'Project Beta',
        type: 'BREAK',
        timeInterval: { start: '2025-01-15T12:00:00Z', end: '2025-01-15T13:00:00Z', duration: 'PT1H' },
        hourlyRate: { amount: 40 },
        billable: false,
      },
    ];
  ...
  it('should produce correct detailed CSV headers (33 columns)', async () => {
    const { downloadDetailedCsv } = await import('../../js/export.js');
    ...
    await downloadDetailedCsv(mockAnalysis);
    ...
    expect(headers[1]).toBe('StartTime');
    expect(headers[2]).toBe('EndTime');
    expect(headers[3]).toBe('User');
    expect(headers[4]).toBe('Project');
    expect(headers[5]).toBe('Client');
    expect(headers[6]).toBe('Task');
    expect(headers[7]).toBe('Description');
    expect(headers[8]).toBe('EntryType');
    expect(headers[9]).toBe('Billable');
    expect(headers[32]).toBe('Status');
  });

  it('should include project, client, task in data rows', async () => {
    const { downloadDetailedCsv } = await import('../../js/export.js');
    ...
    const lines = csvContent.split('\n').filter(l => l.trim());
    // At least one data line should contain project/client/task info
    const allData = lines.slice(1).join('\n');
    expect(allData).toContain('Project Alpha');
    expect(allData).toContain('Client X');
    expect(allData).toContain('Task 1');
    expect(allData).toContain('Alice');
  });
  ...
  it('should include start and end times', async () => {
    ...
    const lines = csvContent.split('\n').filter(l => l.trim());
    const allData = lines.slice(1).join('\n');
    // The time should be formatted as HH:mm (the exact time depends on the test environment's timezone)
    // At minimum, the data should contain time values (not empty)
    expect(allData).toMatch(/\d{2}:\d{2}/);
  });

  it('should use default filename otplus-detailed.csv', async () => {
    const { downloadDetailedCsv } = await import('../../js/export.js');
    ...
    const downloadCall = mockLink.setAttribute.mock.calls.find(c => c[0] === 'download');
    expect(downloadCall[1]).toBe('otplus-detailed.csv');
  });

  it('should derive BREAK status for break entries', async () => {
    const { downloadDetailedCsv } = await import('../../js/export.js');
    ...
    const lines = csvContent.split('\n').filter(l => l.trim());
    const allData = lines.slice(1).join('\n');
    expect(allData).toContain('BREAK');
  });

  it('should sanitize formula injection in descriptions', async () => {
    const { downloadDetailedCsv } = await import('../../js/export.js');
    ...
    const store = createMockStore({ users: [{ id: 'user_1', name: 'Alice' }] });
    const analysis = calculateAnalysis(entries, store, { start: '2025-01-15', end: '2025-01-15' });
    ...
    await downloadDetailedCsv(analysis);

    // The leading = should be prefixed with '
    expect(csvContent).toContain("'=SUM(A1:A10)");
  });

  it('should show Billable as Yes/No', async () => {
    const { downloadDetailedCsv } = await import('../../js/export.js');
    ...
    const lines = csvContent.split('\n').filter(l => l.trim());
    const allData = lines.slice(1).join('\n');
    expect(allData).toContain('Yes');
    expect(allData).toContain('No');
  });
});
