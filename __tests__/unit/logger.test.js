...
describe('Logger Module', () => {
  ...
  describe('Log Level Filtering', () => {
    ...
    it('should always log export audit events even when INFO is below min level', () => {
      loggerModule.setLogLevel(loggerModule.LogLevel.WARN);

      const moduleLogger = loggerModule.createLogger('Export');
      moduleLogger.info('CSV export completed', { action: 'EXPORT_CSV' });
  ...
  describe('Correlation ID handling', () => {
    it('restores the previous correlation ID when a scoped callback throws', () => {
      loggerModule.setCorrelationId('before-throw');

      expect(() => {
        loggerModule.withCorrelationId(() => {
          expect(loggerModule.getCorrelationId()).toBe('throw-id');
          throw new Error('test error');
        }, 'throw-id');
      }).toThrow('test error');

      expect(loggerModule.getCorrelationId()).toBe('before-throw');
      loggerModule.setCorrelationId(null);
    });

    it('includes the current global correlation ID in both text and json output', () => {
      loggerModule.setLogLevel(loggerModule.LogLevel.DEBUG);
      loggerModule.setCorrelationId('global-corr');

      loggerModule.logger.info('text output');
      expect(consoleSpy.log.mock.calls[0][0]).toContain('[global-corr]');

      consoleSpy.log.mockClear();
      loggerModule.setOutputFormat('json');
      loggerModule.logger.info('json output');

      expect(JSON.parse(consoleSpy.log.mock.calls[0][0]).correlationId).toBe('global-corr');

      loggerModule.setCorrelationId(null);
      loggerModule.setOutputFormat('text');
    });

    it('creates a derived logger that uses the provided correlation ID without mutating global state', () => {
      loggerModule.setLogLevel(loggerModule.LogLevel.DEBUG);

      const boundLogger = loggerModule.createLogger('Module').withCorrelationId('bound-corr');
      boundLogger.info('message from correlated logger');

      expect(consoleSpy.log.mock.calls[0][0]).toContain('[bound-corr]');
      expect(consoleSpy.log.mock.calls[0][0]).toContain('[Module]');
      expect(loggerModule.getCorrelationId()).toBeNull();
    });
  });

  describe('Runtime environment guards', () => {
    it('loads without localStorage and still applies runtime config changes', async () => {
      ...
      freshModule.setOutputFormat('json');
      freshModule.enableDebugMode();
    ...
    it('defaults to WARN logging in production when no debug flag is present', async () => {
      ...
      freshModule.createLogger('Test').info('suppressed in production');
      freshModule.createLogger('Test').warn('visible in production');
