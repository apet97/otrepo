...
describe('Negative Path Testing', () => {
  ...
  describe('Aborted Fetch Handling', () => {
    it('should handle AbortError during fetch (no signal → generic throw)', async () => {
      ...
      // No signal provided: fetchUsers throws a generic error (not AbortError)
      await expect(promise).rejects.toThrow('Failed to fetch workspace users');
    });

    it('should re-throw as AbortError when caller signal is aborted', async () => {
      ...
      // Signal is aborted: fetchUsers re-throws as AbortError
    ...
    it('should re-throw as AbortError for pre-aborted signal', async () => {
      ...
      await expect(Api.fetchUsers(MOCK_WORKSPACE_ID, { signal: controller.signal }))
  ...
  describe('HTTP Status Code Handling', () => {
    describe.each(HTTP_STATUS_TEST_CASES.filter(c => !c.ok))(
      ...
      ({ status, description }) => {
        it(`throws on ${status} to surface API failure`, async () => {
          ...
          // fetchUsers throws on non-OK responses so callers can distinguish
          // API failures from genuinely empty workspaces.
          await expect(promise).rejects.toThrow('Failed to fetch workspace users');
        });
      }
    );
  ...
  describe('Network Timeout Handling', () => {
    it('should throw on fetch timeout to surface the failure', async () => {
      ...
      // fetchUsers throws on failure so callers can distinguish API errors
      // from genuinely empty workspaces.
      await expect(Api.fetchUsers(MOCK_WORKSPACE_ID)).rejects.toThrow('Failed to fetch workspace users');
  ...
  describe('Edge Case Input Handling', () => {
    ...
    it('should throw on undefined workspace ID (URL validation fails)', async () => {
      // Temporarily clear store claims
      store.claims = null;
      store.token = null;

      // URL construction with undefined backendUrl produces an invalid URL,
      // which fetchWithAuth blocks (returns failed: true, status: 400).
