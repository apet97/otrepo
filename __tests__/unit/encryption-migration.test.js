...
describe('Encryption Migration', () => {
  ...
  it('falls back to defaults when encrypted overrides are tampered', async () => {
    ...
    // Flip several characters to guarantee the ciphertext is actually corrupted
    // (single-char swap is a no-op if ct already starts with that character)
    const flipped = encrypted.ct.split('').map((c, i) => i < 4 ? (c === 'A' ? 'B' : 'A') : c).join('');
    const tampered = { ...encrypted, ct: flipped };
