    ...
    describe('MemoryProfiler class', () => {
        ...
        describe('start', () => {
            ...
            it('starts interval when autoSample is true', () => {
                ...
                // Profiler should be running with the interval active
                expect(profiler.getStatus().isRunning).toBe(true);

                // Advance timers — interval fires but no samples collected (memory API unavailable)
                jest.advanceTimersByTime(300);
                expect(profiler.getStatus().sampleCount).toBe(0);
        ...
        describe('takeSnapshot', () => {
            ...
            it('accepts optional label', () => {
                // Memory API is not available, so snapshot should be null
                const snapshot = profiler.takeSnapshot('test-label');
                expect(snapshot).toBeNull();

                // Label should NOT be stored when snapshot is null (early return)
