...
describe('Metrics Module', () => {
    ...
    describe('startTimer', () => {
        ...
        it('allows adding labels to the metric', () => {
            ...
            // Metric recorded under labeled key (name + JSON-serialized labels)
            const stats = getHistogramStats('test_labels:{"status":"success","method":"GET"}');
            expect(stats).not.toBeNull();
            expect(stats.count).toBe(1);

            // Not recorded under the base name alone
            const baseStats = getHistogramStats('test_labels');
            expect(baseStats).toBeNull();
        });
    });
    ...
    describe('recordHistogram', () => {
        ...
        it('logs value when autoLog is enabled', () => {
            ...
            const stats = getHistogramStats('logged_metric');
            expect(stats).not.toBeNull();
            expect(stats.count).toBe(1);
        });
