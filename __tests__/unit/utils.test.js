...
describe('Branch Coverage - Utils Functions', () => {
  ...
  describe('round edge cases (line 334)', () => {
    ...
    it('should handle rounding edge cases for large values and halves', () => {
      // IEEE-754 representation means some .005 values are slightly below the midpoint.
...
describe('escapeCsv Mutation Tests', () => {
  ...
  describe('basic escaping', () => {
    ...
    it('should convert number to string', () => {
      ...
      // Negative numbers are quoted to prevent formula injection (- is a formula char)
...
describe('isTokenExpired', () => {
  ...
  it('returns true when exp is missing', () => {
    expect(isTokenExpired(makeJwt({ sub: 'user1' }))).toBe(true);
  });
  ...
  it('returns true when payload is invalid JSON', () => {
    expect(isTokenExpired('header.!!!.signature')).toBe(true);
  });
});

describe('isAllowedClockifyUrl — shared URL trust contract', () => {
  it('allows HTTPS Clockify API URLs', () => {
    ...
    expect(isAllowedClockifyUrl('https://eu.api.clockify.me/api')).toBe(true);
    expect(isAllowedClockifyUrl('https://reports.api.clockify.me')).toBe(true);
    expect(isAllowedClockifyUrl('https://clockify.me')).toBe(true);
  });

  it('rejects HTTP Clockify URLs (HTTPS enforcement)', () => {
    expect(isAllowedClockifyUrl('http://api.clockify.me')).toBe(false);
    expect(isAllowedClockifyUrl('http://clockify.me')).toBe(false);
  });

  it('rejects non-Clockify domains', () => {
  ...
  it('rejects malformed URLs', () => {
    ...
    expect(isAllowedClockifyUrl('://broken')).toBe(false);
  });
});
