...
const getDefaultConfig = (): LoggerConfig => {
    ...
    const isDebug = hasLocalStorage && localStorage.getItem(STORAGE_KEYS.DEBUG) === 'true';
 ...
 */
const AUDIT_ACTIONS = ['CONFIG_CHANGE', 'OVERRIDE_CHANGE', 'EXPORT', 'EXPORT_CSV'] as const;
    ...
    return data.some((item) => {
        if (item && typeof item === 'object' && 'action' in item) {
            ...
            return (
                ...
                AUDIT_ACTIONS.includes(action as (typeof AUDIT_ACTIONS)[number])
            );
        }
        return false;
    });
...
export function createLogger(module: string, correlationId?: string) {
    ...
    return {
        ...
        error: (message: string, ...data: unknown[]) =>
            logWithContext(LogLevel.ERROR, message, ...data),
        /**
...
export const logger = {
    debug: (message: string, ...data: unknown[]) =>
        log(LogLevel.DEBUG, undefined, message, ...data),
    ...
    error: (message: string, ...data: unknown[]) =>
        log(LogLevel.ERROR, undefined, message, ...data),
    log: (level: LogLevel, message: string, ...data: unknown[]) =>
        log(level, undefined, message, ...data),
