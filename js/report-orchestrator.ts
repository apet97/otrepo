...
export async function handleGenerateReport(forceRefresh = false): Promise<void> {
    ...
    if (cacheKey && !forceRefresh) {
        ...
        if (cached) {
            ...
            if (cacheAgeSeconds <= 5) {
                // Very fresh cache — auto-reuse without prompting (just fetched)
                useCachedData = true;
            } else {
    ...
    try {
            ...
            if (missingUsers.length > 0) {
                optionalPromises.push({
                    ...
                    promise: Api.fetchAllProfiles(store.claims.workspaceId, missingUsers, {
                        signal,
                        onProgress: (fetched, phase) =>
                            UI.updateLoadingProgress(fetched, phase, missingUsers.length),
                    }).then(async (profiles) => {
                        if (thisRequestId !== currentRequestId) {
                            return;
                        }
                        profiles.forEach((profile, userId) => {
                            store.setProfile(userId, {
        ...
        const optionalResultsPromise =
            optionalPromises.length > 0
