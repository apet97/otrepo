    ...
    metrics.forEach((metric, key) => {
        ...
        if (metric.type === 'histogram') {
            ...
            if (stats) {
                ...
                lines.push(
                    `${safeName}_bucket{le="50"} ${metric.values.filter((v) => v <= 50).length}`
                );
                lines.push(
                    `${safeName}_bucket{le="100"} ${metric.values.filter((v) => v <= 100).length}`
                );
                lines.push(
                    `${safeName}_bucket{le="500"} ${metric.values.filter((v) => v <= 500).length}`
                );
                lines.push(
                    `${safeName}_bucket{le="1000"} ${metric.values.filter((v) => v <= 1000).length}`
                );
...
export function getMetricsSummary(): Record<string, unknown> {
    ...
    metrics.forEach((metric, key) => {
        if (metric.type === 'histogram') {
            const stats = getHistogramStats(key);
            summary[key] = stats
                ? `avg=${stats.avg.toFixed(2)}ms, p95=${stats.p95.toFixed(2)}ms, count=${stats.count}`
                : 'no data';
        } else if (metric.type === 'counter') {
