...
import { IsoUtils, classifyError, base64urlDecode, isAllowedClockifyUrl } from './utils.js';
import {
    DEFAULT_MAX_PAGES,
...
export function sanitizeUrlForLogging(url: string): string {
    ...
    sanitized = sanitized.replace(/\/workspaces\/[^/?]+/gi, '/workspaces/[REDACTED]');

    // Replace user IDs in path: /user/{id} or /users/{id}
    sanitized = sanitized.replace(/\/users?\/[^/?]+/gi, (match) =>
        match.startsWith('/users') ? '/users/[REDACTED]' : '/user/[REDACTED]'
    );

    // Replace member-profile IDs: /member-profile/{id}
    sanitized = sanitized.replace(/\/member-profile\/[^/?]+/gi, '/member-profile/[REDACTED]');
...
function recordFailure(isRetryable: boolean): void {
    ...
    if (newState.state === 'HALF_OPEN') {
        ...
        newState.successCount = 0;
        newState.halfOpenPending = false;
        newState.nextRetryTime =
            Date.now() +
            (deps().getConfig().circuitBreakerResetMs ?? CIRCUIT_BREAKER_CONFIG.resetTimeout);
        // Single atomic reference swap
        circuitBreaker = newState;
        ...
        newState.state === 'CLOSED' &&
        newState.failureCount >=
            (deps().getConfig().circuitBreakerFailureThreshold ??
...
// Enforces HTTPS + *.clockify.me hostname. Same contract as main.ts and worker/src/auth.ts.
...
async function fetchWithAuth<T>(
    ...
    if (!isAllowedClockifyUrl(url)) {
        apiLogger.error('Blocked request to untrusted or invalid API URL', {
    ...
    async function waitForToken(): Promise<void> {
        while (true) {
            ...
            const waitTime =
                (deps().getConfig().rateLimitRefillMs ?? REFILL_INTERVAL) - (now - lastRefill);
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        ...
        try {
            ...
            if (errorType === 'AUTH_ERROR' || errorType === 'VALIDATION_ERROR') {
                apiLogger.error('Fetch error (not retryable)', {
                    errorType,
                    message: err?.message,
                });
                // Don't record as circuit breaker failure - these are not service issues
...
export const Api = {
    ...
    async fetchUsers(workspaceId: string, options?: FetchOptions | AbortSignal): Promise<User[]> {
        ...
        while (true) {
            ...
            const { data, failed, status } = await fetchWithAuth<User[]>(
                `${deps().getClaims()?.backendUrl}${BASE_API}/${workspaceId}/users?page=${page}&page-size=${pageSize}`,
                fetchOpts
            );
            if (failed) {
                // Re-throw as AbortError if the caller's signal was aborted — this is
                // user-initiated cancellation, not an API failure.
                if (fetchOpts.signal?.aborted) {
                    const abortErr = new Error('aborted');
                    abortErr.name = 'AbortError';
                    throw abortErr;
                }
                // Surface transport/auth failures explicitly so callers can distinguish
                // a real API failure from a genuinely empty workspace.
                throw new Error(`Failed to fetch workspace users (HTTP ${status})`);
            }
            ...
            if (page > HARD_MAX_PAGES_LIMIT) {
                apiLogger.warn('Reached page limit for user fetch', {
                    page,
    ...
    async fetchDetailedReport(
        ...
        const normalizeAmounts = (
            ...
            if (raw && typeof raw === 'object') {
                ...
                if ('type' in raw || 'amountType' in raw || 'value' in raw || 'amount' in raw) {
        ...
        while (hasMore) {
            ...
            if (options.signal?.aborted) {
                apiLogger.info('Pagination aborted by caller before page fetch', {
                    page,
            ...
            if (failed || !data) {
                // Stryker disable next-line StringLiteral: Log message is not testable
                apiLogger.error('Detailed report fetch failed', {
                    page,
                    entriesFetched: allEntries.length,
                });
            ...
            const transformed: TimeEntry[] = entries.map((e) => {
                ...
                const resolvedHourlyRate = pickRateValue(e.earnedRate, e.rate, e.hourlyRate);
                ...
                const hourlyRateCurrency =
                    typeof e.hourlyRate === 'object' && e.hourlyRate && 'currency' in e.hourlyRate
            ...
            if (options.signal?.aborted) {
                apiLogger.info('Pagination aborted after transformation', {
                    page,
                    entriesFetched: allEntries.length,
                });
                return allEntries;
            ...
            if (entries.length < pageSize) {
                ...
                if (page > effectiveMaxPages) {
                    console.warn(
                        `Reached page limit (${effectiveMaxPages}), stopping pagination. Total entries fetched: ${allEntries.length}`
                    );
                    deps().setUiPaginationFlag('paginationTruncated');
                    hasMore = false;
                }
    ...
    async fetchTimeOffRequests(
        ...
        while (true) {
            ...
            const requests =
                rawData.requests ||
                rawData.timeOffRequests ||
                (Array.isArray(data) ? (data as TimeOffRequest[]) : []);
            ...
            if (page > HARD_MAX_PAGES_LIMIT) {
                apiLogger.warn('Reached page limit for time-off requests', {
                    page,
        ...
        return {
            data: { requests: allRequests } as RawTimeOffResponse,
            failed: false,
            status: 200,
        };
    },
    ...
    async fetchAllProfiles(
        ...
        const failedUserIds = users.filter((user) => !results.has(user.id)).map((user) => user.id);
    ...
    async fetchAllHolidays(
        ...
        // Helper: fetch all remaining unsampled users individually
        const fetchRemainingUsers = async (remaining: User[]): Promise<void> => {
            for (const batch of splitIntoBatches(remaining, BATCH_SIZE)) {
                const batchPromises = batch.map(async (user) => {
                    const { data, failed } = await this.fetchHolidays(
                        workspaceId,
                        user.id,
        ...
        // If only ONE pattern found across all samples, verify before propagating
        if (holidayPatterns.size === 1 && users.length > SAMPLE_SIZE) {
            const VERIFICATION_SIZE = 3;
            const unsampledUsers = users.filter((u) => !sampledUserIds.has(u.id));
            const verifyUsers = unsampledUsers.slice(
                0,
                Math.min(VERIFICATION_SIZE, unsampledUsers.length)
            );
            const [referenceHash] = [...holidayPatterns.keys()];
            let verified = true;
            ...
            for (const user of verifyUsers) {
                ...
                sampledUserIds.add(user.id);
                if (failed) {
                    failedCount++;
                    verified = false;
                    break;
                }
                const normalized = data ? normalizeHolidays(data) : [];
                results.set(user.id, normalized);
                const hash = JSON.stringify(normalized);
                if (!holidayPatterns.has(hash)) holidayPatterns.set(hash, normalized);
                // eslint-disable-next-line security/detect-possible-timing-attacks -- holiday pattern hash, not crypto
            ...
            if (verified) {
                ...
                apiLogger.info('Holiday deduplication: propagating identical holidays', {
                    ...
                    verificationSize: verifyUsers.length,
                    propagatedTo: users.length - sampledUserIds.size,
                });
                ...
                // Verification failed — fetch all remaining unsampled users
                await fetchRemainingUsers(users.filter((u) => !sampledUserIds.has(u.id)));
            }
        } else {
    ...
    async fetchAllTimeOff(
        ...
        const userIdBatches = splitIntoBatches(
            users.map((u) => u.id),
            TIME_OFF_USER_BATCH_SIZE
        );
        let allRequests: TimeOffRequest[] = [];
        let anyFailed = false;
        // Track which user IDs were in failed batches (not which users had no requests)
        const failedBatchUserIds = new Set<string>();
        ...
        for (const userIdBatch of userIdBatches) {
            ...
            if (failed) {
                anyFailed = true;
                // Record all users from this failed batch
                userIdBatch.forEach((id) => failedBatchUserIds.add(id));
                continue;
            }
        ...
        if (anyFailed) {
            ...
            // Count users from failed batches, NOT absence from returned requests
            // (a successful batch can return zero requests for users with no approved time-off).
            const failedCount = failedBatchUserIds.size;
        ...
        requests.forEach((request) => {
            ...
            const statusType =
                typeof request.status === 'object' ? request.status.statusType : request.status;
 ...
 * Calls the Clockify API to verify the user's membership type.
 */
export async function checkIsAdmin(userId: string): Promise<boolean> {
    const d = deps();
    const token = d.getToken();
    const claims = d.getClaims();
    if (!token || !claims?.workspaceId || !claims?.backendUrl) return false;

    try {
        const url = `${claims.backendUrl}/v1/workspaces/${claims.workspaceId}/users/${userId}`;
        ...
        const membership = user.memberships?.find((m) => m.membershipStatus === 'ACTIVE');
        return (
            membership?.membershipType === 'WORKSPACE_ADMIN' ||
