...
import { getHistogramStats, getCounterValue, MetricNames } from './metrics.js';
...
function unrefIntervalIfSupported(intervalId: ReturnType<typeof setInterval> | null): void {
    if (!intervalId || typeof intervalId !== 'object') return;
    const maybeInterval = intervalId as { unref?: () => void };
    if (typeof maybeInterval.unref === 'function') {
        maybeInterval.unref();
    }
}
...
export function generatePerformanceReport(): PerformanceReport {
    ...
    if (memory && memory.usedHeapMB > config.alertThresholds.memoryUsageMB) {
        alerts.push({
            type:
                memory.usedHeapMB > config.alertThresholds.memoryUsageMB * 1.5
                    ? 'critical'
                    : 'warning',
            metric: 'memory',
    ...
    if (apiLatency && apiLatency.p95 > config.alertThresholds.apiLatencyP95Ms) {
        alerts.push({
            type:
                apiLatency.p95 > config.alertThresholds.apiLatencyP95Ms * 2
                    ? 'critical'
                    : 'warning',
            metric: 'api_latency',
    ...
    if (calcLatency && calcLatency.p95 > config.alertThresholds.calcTimeP95Ms) {
        alerts.push({
            type:
                calcLatency.p95 > config.alertThresholds.calcTimeP95Ms * 2 ? 'critical' : 'warning',
            metric: 'calc_time',
...
function formatReportForConsole(report: PerformanceReport): string {
    ...
    if (report.memory) {
        lines.push('║  MEMORY                                                      ║');
        lines.push(
            `║    Heap: ${report.memory.usedHeapMB.toFixed(1)}MB / ${report.memory.heapLimitMB.toFixed(1)}MB (${report.memory.usagePercent}%)`.padEnd(
                64
            ) + '║'
        );
    }
    ...
    lines.push(
        `║    Requests: ${report.api.totalRequests}, Errors: ${report.api.errors}, Retries: ${report.api.retries}`.padEnd(
            64
        ) + '║'
    );
    ...
    if (report.api.latency) {
        lines.push(
            `║    Latency: avg=${report.api.latency.avg.toFixed(0)}ms, p95=${report.api.latency.p95.toFixed(0)}ms, p99=${report.api.latency.p99.toFixed(0)}ms`.padEnd(
                64
            ) + '║'
        );
    }
    ...
    lines.push(
        `║    Total: ${report.calculations.totalCalculations}, Entries: ${report.calculations.entriesProcessed}`.padEnd(
            64
        ) + '║'
    );
    if (report.calculations.latency) {
        lines.push(
            `║    Duration: avg=${report.calculations.latency.avg.toFixed(0)}ms, p95=${report.calculations.latency.p95.toFixed(0)}ms`.padEnd(
                64
            ) + '║'
        );
    }
    ...
    lines.push(
        `║    State: ${report.circuitBreaker.state}, Failures: ${report.circuitBreaker.failures}`.padEnd(
            64
        ) + '║'
    );
    ...
    const alertsHtml =
        report.alerts.length > 0
            ? report.alerts
                  .map(
                      (a) =>
                          `<div style="color: ${a.type === 'critical' ? '#f00' : '#ff0'}">⚠ ${escapeHtml(a.message)}</div>`
                  )
                  .join('')
